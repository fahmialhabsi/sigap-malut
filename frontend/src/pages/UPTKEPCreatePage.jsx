...
export default function UPTKEPCreatePage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await api.post("/upt-kep", {
        unit_kerja: formData.unit_kerja,
        akses_terbatas: formData.akses_terbatas === "true",
        hak_akses_uptd: formData.hak_akses_uptd,
      });

      alert("Data Kepegawaian UPTD berhasil dibuat.");
      navigate("/module/upt-kep");
    } catch (error) {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Hak Akses UPTD <span className="text-red-500">*</span>
            </label>
            <select
              name="hak_akses_uptd"
              value={formData.hak_akses_uptd}
              onChange={handleChange}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 text-gray-900 focus:ring-blue-500"
              required
            >
        ...
        <div className="mt-6 flex gap-4">
          ...
          <button
            type="button"
            onClick={() => navigate("/module/upt-kep")}
