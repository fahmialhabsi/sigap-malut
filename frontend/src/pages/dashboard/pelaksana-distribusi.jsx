// frontend/src/pages/dashboard/pelaksana-distribusi.jsx
// Role: pelaksana | Unit: Bidang Distribusi Pangan
...
const ALLOWED = ["pelaksana", "staf", "super_admin", "kepala_dinas", "kepala_bidang_distribusi", "fungsional_distribusi"];
...
const MODUL_BDS = [
  { kode: "BDS-BMB", label: "Bahan Makanan Bergizi", icon: "🍱" },
  { kode: "BDS-CPD", label: "Cadangan Pangan Daerah", icon: "🏪" },
  { kode: "BDS-EVL", label: "Evaluasi Distribusi", icon: "📊" },
  { kode: "BDS-HRG", label: "Harga & Pasar", icon: "💹" },
  { kode: "BDS-KBJ", label: "Kebijakan Distribusi", icon: "📜" },
  { kode: "BDS-LAP", label: "Laporan Distribusi", icon: "📋" },
  { kode: "BDS-MON", label: "Monitoring Distribusi", icon: "🔍" },
];

export default function PelaksanaDistribusiDashboard() {
  ...
  const fetchTasks = useCallback(() => {
    ...
    api.get("/tasks?unit=distribusi&limit=20")
  ...
  return (
    <div style={{ background: T.bg, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <header role="banner" style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.secondary})`, borderBottom: `3px solid ${T.accent}` }}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11 }}>Pelaksana — Bidang Distribusi Pangan</div>
          </div>
        </div>
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        {tab === "ringkasan" && (
          <>
            <div style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.secondary})`, borderRadius: 14, padding: "20px 24px", color: "#fff" }}>
              ...
              <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 12, marginTop: 3 }}>Bidang Distribusi Pangan · {user?.name || user?.username || "—"}</p>
            </div>
        ...
        {tab === "tugas" && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.textPri, marginBottom: 14 }}>📋 Tugas Bidang Distribusi</h2>
        ...
        {tab === "modul" && (
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.textPri, marginBottom: 14 }}>🗂️ Modul Bidang Distribusi</h2>
        ...
        {tab === "notifikasi" && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24 }}>
            ...
            {notifList.length === 0 ? <p style={{ color: T.textSec, fontSize: 13, fontStyle: "italic" }}>Tidak ada notifikasi.</p>
              : notifList.map((n, i) => (
                <div key={n.id ?? i} style={{ background: n.read_at ? "#F9FAFB" : "#EFF6FF", border: `1px solid ${n.read_at ? T.border : "#93C5FD"}`, borderRadius: 10, padding: "11px 14px", marginBottom: 8 }}>
      ...
      <footer role="contentinfo" style={{ textAlign: "center", fontSize: 11, color: T.textSec, padding: "12px 0", borderTop: `1px solid ${T.border}`, marginTop: 24 }}>
        SIGAP-MALUT © 2026 · Pelaksana Bidang Distribusi Pangan
      </footer>
    </div>
  );
}
