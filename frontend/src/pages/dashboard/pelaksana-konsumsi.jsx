// frontend/src/pages/dashboard/pelaksana-konsumsi.jsx
// Role: pelaksana | Unit: Bidang Konsumsi Pangan
...
const T = {
  primary: "#6A1B9A", secondary: "#4A148C", accent: "#F57C00",
  danger: "#C62828", success: "#2E7D32", warning: "#E65100",
  bg: "#F3E5F5", card: "#FFFFFF", border: "#E1BEE7",
  textPri: "#1A0A2B", textSec: "#6A3D8A",
};

const ALLOWED = ["pelaksana", "staf", "super_admin", "kepala_dinas", "kepala_bidang_konsumsi", "fungsional_konsumsi"];
...
const MODUL_BKS = [
  { kode: "BKS-BMB", label: "Bahan Makanan Bergizi", icon: "🍽️" },
  { kode: "BKS-DVR", label: "Diversifikasi Pangan", icon: "🌈" },
  { kode: "BKS-EVL", label: "Evaluasi Konsumsi", icon: "📊" },
  { kode: "BKS-KBJ", label: "Kebijakan Konsumsi", icon: "📜" },
  { kode: "BKS-KMN", label: "Keamanan Mutu Pangan", icon: "🛡️" },
  { kode: "BKS-LAP", label: "Laporan Konsumsi", icon: "📋" },
];

export default function PelaksanaKonsumsiDashboard() {
  ...
  const fetchTasks = useCallback(() => {
    ...
    api.get("/tasks?unit=konsumsi&limit=20")
    ...
    <div style={{ background: T.bg, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <header role="banner" style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.secondary})`, borderBottom: `3px solid ${T.accent}` }}
        ...
        <div className="flex items-center gap-3">
          <div style={{ background: T.accent, width: 36, height: 36, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>🍽️</div>
          <div>
            <div style={{ color: "#fff", fontWeight: 800, fontSize: 15 }}>SIGAP MALUT</div>
            <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11 }}>Pelaksana — Bidang Konsumsi Pangan</div>
          </div>
        </div>
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        {tab === "ringkasan" && (
          <>
            <div style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.secondary})`, borderRadius: 14, padding: "20px 24px", color: "#fff" }}>
              <h1 style={{ fontSize: 20, fontWeight: 800 }}>Dashboard Pelaksana</h1>
              <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 12, marginTop: 3 }}>Bidang Konsumsi Pangan · {user?.name || user?.username || "—"}</p>
            </div>
        ...
        {tab === "tugas" && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.textPri, marginBottom: 14 }}>📋 Tugas Bidang Konsumsi</h2>
        ...
        {tab === "modul" && (
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.textPri, marginBottom: 14 }}>🗂️ Modul Bidang Konsumsi</h2>
        ...
        {tab === "notifikasi" && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24 }}>
            ...
            {notifList.length === 0 ? <p style={{ color: T.textSec, fontSize: 13, fontStyle: "italic" }}>Tidak ada notifikasi.</p>
              : notifList.map((n, i) => (
                <div key={n.id ?? i} style={{ background: n.read_at ? "#F9FAFB" : "#F5F3FF", border: `1px solid ${n.read_at ? T.border : "#C4B5FD"}`, borderRadius: 10, padding: "11px 14px", marginBottom: 8 }}>
      ...
      <footer role="contentinfo" style={{ textAlign: "center", fontSize: 11, color: T.textSec, padding: "12px 0", borderTop: `1px solid ${T.border}`, marginTop: 24 }}>
        SIGAP-MALUT © 2026 · Pelaksana Bidang Konsumsi Pangan
      </footer>
    </div>
  );
}
