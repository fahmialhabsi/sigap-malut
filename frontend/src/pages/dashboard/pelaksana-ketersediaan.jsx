// frontend/src/pages/dashboard/pelaksana-ketersediaan.jsx
// Role: pelaksana | Unit: Bidang Ketersediaan & Kerawanan Pangan
...
const T = {
  primary: "#2E7D32", secondary: "#1B5E20", accent: "#F57C00",
  danger: "#C62828", success: "#2E7D32", warning: "#E65100",
  bg: "#F1F8E9", card: "#FFFFFF", border: "#C8E6C9",
  textPri: "#0A1F0B", textSec: "#3B6B3D",
};

const ALLOWED = ["pelaksana", "staf", "super_admin", "kepala_dinas", "kepala_bidang_ketersediaan", "fungsional_ketersediaan"];
...
const MODUL_BKT = [
  { kode: "BKT-BMB", label: "Bahan Makanan Bergizi", icon: "🌾" },
  { kode: "BKT-FSL", label: "Fasilitasi Sumber Pangan", icon: "🏗️" },
  { kode: "BKT-KBJ", label: "Kebijakan Ketersediaan", icon: "📜" },
  { kode: "BKT-KRW", label: "Kerawanan Pangan", icon: "⚠️" },
  { kode: "BKT-MEV", label: "Monitoring & Evaluasi", icon: "📊" },
  { kode: "BKT-PGD", label: "Pengadaan & Distribusi", icon: "🚚" },
];

export default function PelaksanaKetersediaanDashboard() {
  ...
  const fetchTasks = useCallback(() => {
    ...
    api.get("/tasks?unit=ketersediaan&limit=20")
  ...
  return (
    <div style={{ background: T.bg, minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <header role="banner" style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.secondary})`, borderBottom: `3px solid ${T.accent}` }}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 11 }}>Pelaksana — Bidang Ketersediaan & Kerawanan Pangan</div>
          </div>
        </div>
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        {tab === "ringkasan" && (
          <>
            <div style={{ background: `linear-gradient(135deg, ${T.primary}, ${T.secondary})`, borderRadius: 14, padding: "20px 24px", color: "#fff" }}>
              ...
              <p style={{ color: "rgba(255,255,255,0.8)", fontSize: 12, marginTop: 3 }}>Bidang Ketersediaan & Kerawanan Pangan · {user?.name || user?.username || "—"}</p>
            </div>
        ...
        {tab === "tugas" && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24 }}>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.textPri, marginBottom: 14 }}>📋 Tugas Bidang Ketersediaan</h2>
        ...
        {tab === "modul" && (
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: T.textPri, marginBottom: 14 }}>🗂️ Modul Bidang Ketersediaan</h2>
        ...
        {tab === "notifikasi" && (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 24 }}>
            ...
            {notifList.length === 0 ? <p style={{ color: T.textSec, fontSize: 13, fontStyle: "italic" }}>Tidak ada notifikasi.</p>
              : notifList.map((n, i) => (
                <div key={n.id ?? i} style={{ background: n.read_at ? "#F9FAFB" : "#F0FDF4", border: `1px solid ${n.read_at ? T.border : "#86EFAC"}`, borderRadius: 10, padding: "11px 14px", marginBottom: 8 }}>
      ...
      <footer role="contentinfo" style={{ textAlign: "center", fontSize: 11, color: T.textSec, padding: "12px 0", borderTop: `1px solid ${T.border}`, marginTop: 24 }}>
        SIGAP-MALUT © 2026 · Pelaksana Bidang Ketersediaan & Kerawanan Pangan
      </footer>
    </div>
  );
}
