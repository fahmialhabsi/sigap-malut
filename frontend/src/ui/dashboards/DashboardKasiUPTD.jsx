...
export default function DashboardKasiUPTD() {
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      setLoading(true);
      // Ambil unit_id dari user (prioritaskan user.unit_id, fallback ke user.unit_kerja jika numerik)
  ...
  // Panel notifikasi broadcast UPTD (pindahkan ke atas agar hooks tidak dipanggil kondisional)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="mb-4">
        {notifLoading ? (
          <div className="bg-teal-900/80 border border-teal-400/30 rounded-xl p-4 text-teal-200 text-sm animate-pulse">
            Memuat notifikasi…
          </div>
        ) : notifList.filter((n) =>
            n.message?.startsWith("[Broadcast Kepala UPTD]"),
          ).length === 0 ? null : (
          <div className="bg-teal-900/80 border border-teal-400/30 rounded-xl p-4 text-teal-100 text-sm space-y-2">
