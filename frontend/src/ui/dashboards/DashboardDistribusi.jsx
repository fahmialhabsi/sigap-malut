  ...
  return (
    <DashboardDistribusiLayout fallbackModules={_distribusiModules}>
      ...
      <div className="px-6 md:px-12 pb-8">
        ...
        <div className="mt-8 bg-blue-950/80 border-2 border-blue-700/50 rounded-2xl p-7 shadow-xl" style={{ backdropFilter: "blur(17px)" }}>
          ...
          {subKegListLoading ? (
            <p className="text-xs text-blue-300/70 animate-pulse">Memuat data efektivitas…</p>
          ) : (() => {
            const total = subKegList.length;
            const selesai = subKegList.filter((d) => d.status === "selesai" || d.status === "disetujui").length;
            const pending = subKegList.filter((d) => !d.status || d.status === "diajukan" || d.status === "pending").length;
              ...
              <div>
                <div className="mb-5">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs text-blue-300">% Sub-Kegiatan Selesai</span>
                    <span className="text-lg font-bold text-white">{total > 0 ? `${pct}%` : "—"}</span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-green-900/30 border border-green-700/40 rounded-xl p-4 text-center">
                    <div className="text-2xl font-bold text-green-300">{selesai}</div>
                    <div className="text-xs text-green-400/80 mt-1">Selesai / Disetujui</div>
                  </div>
                  <div className="bg-amber-900/30 border border-amber-700/40 rounded-xl p-4 text-center">
                    ...
                    <div className="text-xs text-amber-400/80 mt-1">Pending</div>
                  </div>
                  <div className="bg-red-900/30 border border-red-700/40 rounded-xl p-4 text-center">
                    <div className="text-2xl font-bold text-red-300">{ditolak}</div>
                    <div className="text-xs text-red-400/80 mt-1">Ditolak</div>
                  </div>
                </div>
                {total === 0 && (
                  <p className="text-xs text-blue-300/60 mt-4">Belum ada sub-kegiatan yang diusulkan untuk Bidang Distribusi.</p>
                )}
              </div>
            );
          })()}
