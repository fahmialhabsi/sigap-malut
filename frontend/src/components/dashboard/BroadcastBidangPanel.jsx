...
/**
 * Panel broadcast/perintah generik untuk Kepala Bidang (Ketersediaan, Distribusi, Konsumsi)
 * @param {Array<{value: string, label: string}>} tujuanOptions - daftar tujuan (role/unit)
 * @param {string} endpoint - endpoint API broadcast
 * @param {string} [label] - label panel
 */
export default function BroadcastBidangPanel({
  tujuanOptions,
  endpoint,
  label,
}) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {label && (
        <div className="font-bold text-green-200 text-base mb-1">{label}</div>
      )}
      <select
        value={tujuan}
