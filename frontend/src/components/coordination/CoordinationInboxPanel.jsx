import React, { useEffect, useRef, useState } from "react";
import toast from "react-hot-toast";
import coordinationService from "../../services/coordinationService";
import { getRoleLabel } from "./coordinationOptions";

function kindBadge(kind) {
  return kind === "perintah"
    ? "bg-blue-50 text-blue-700 border border-blue-200"
    : "bg-amber-50 text-amber-700 border border-amber-200";
}

function statusBadge(status) {
  const normalized = String(status || "").toLowerCase();
  if (normalized === "submitted") {
    return "bg-emerald-50 text-emerald-700 border border-emerald-200";
  }
  if (normalized === "closed") {
    return "bg-slate-100 text-slate-700 border border-slate-200";
  }
  return "bg-gray-50 text-gray-700 border border-gray-200";
}

export default function CoordinationInboxPanel({
  title,
  subtitle,
  sourceRole,
  kind,
  refreshKey,
  emptyText = "Belum ada item masuk.",
  showRespondForm = true,
  allowClose = false,
  panelClassName = "",
  highlightTaskId = null,
}) {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(false);
  const [notes, setNotes] = useState({});
  const [actingId, setActingId] = useState(null);
  const didScrollRef = useRef(false);

  async function load() {
    setLoading(true);
    try {
      const res = await coordinationService.listInbox({
        limit: 40,
        ...(sourceRole ? { source_role: sourceRole } : {}),
        ...(kind ? { kind } : {}),
      });
      setRows(Array.isArray(res.data?.data) ? res.data.data : []);
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Gagal memuat inbox koordinasi.",
      );
      setRows([]);
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => {
    load();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sourceRole, kind, refreshKey]);

  useEffect(() => {
    didScrollRef.current = false;
  }, [highlightTaskId]);

  useEffect(() => {
    if (!highlightTaskId || rows.length === 0) return;
    if (didScrollRef.current === highlightTaskId) return;
    const el = document.getElementById(`coordination-task-${highlightTaskId}`);
    if (el) {
      el.scrollIntoView({ behavior: "smooth", block: "center" });
      didScrollRef.current = highlightTaskId;
    }
  }, [highlightTaskId, rows]);

  async function handleRespond(id) {
    const note = String(notes[id] || "").trim();
    if (!note) {
      toast.error("Tanggapan wajib diisi.");
      return;
    }

    setActingId(id);
    try {
      await coordinationService.respond(id, { response_note: note });
      toast.success("Tanggapan berhasil dikirim.");
      setNotes((prev) => ({ ...prev, [id]: "" }));
      await load();
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Gagal mengirim tanggapan.",
      );
    } finally {
      setActingId(null);
    }
  }

  async function handleClose(id) {
    setActingId(id);
    try {
      await coordinationService.close(id, {});
      toast.success("Item koordinasi ditutup.");
      await load();
    } catch (error) {
      toast.error(
        error?.response?.data?.message || "Gagal menutup item koordinasi.",
      );
    } finally {
      setActingId(null);
    }
  }

  return (
    <section
      className={`bg-white rounded-xl border border-gray-100 shadow-sm p-5 ${panelClassName}`}
    >
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="font-bold text-gray-800">{title}</h2>
          {subtitle ? <p className="text-xs text-gray-500 mt-1">{subtitle}</p> : null}
        </div>
        <button
          type="button"
          onClick={load}
          className="text-xs px-3 py-1.5 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray-100"
        >
          Refresh
        </button>
      </div>

      {loading ? (
        <div className="text-sm text-gray-400 py-10 text-center animate-pulse">
          Memuat...
        </div>
      ) : rows.length === 0 ? (
        <div className="text-sm text-gray-400 py-10 text-center">{emptyText}</div>
      ) : (
        <div className="space-y-3">
          {rows.map((row) => {
            const meta = row.metadata || {};
            const latestResponse = meta.latest_response_note;
            const canRespond = showRespondForm && row.status !== "closed";
            const isHi =
              highlightTaskId != null && Number(row.id) === Number(highlightTaskId);
            return (
              <div
                key={row.id}
                id={`coordination-task-${row.id}`}
                className={`border rounded-xl p-4 hover:border-emerald-200 transition ${
                  isHi
                    ? "border-emerald-400 ring-2 ring-emerald-200 bg-emerald-50/40"
                    : "border-gray-100"
                }`}
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${kindBadge(
                          meta.kind,
                        )}`}
                      >
                        {String(meta.kind || "koordinasi").toUpperCase()}
                      </span>
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${statusBadge(
                          row.status,
                        )}`}
                      >
                        {String(row.status || "assigned").toUpperCase()}
                      </span>
                    </div>
                    <div className="font-semibold text-gray-800 text-sm">
                      {row.title}
                    </div>
                    <div className="text-[11px] text-gray-500 mt-1">
                      Dari: {getRoleLabel(meta.from_role)} | Unit:{" "}
                      {row.source_unit || "-"} | Deadline:{" "}
                      {row.due_date
                        ? new Date(row.due_date).toLocaleDateString("id-ID")
                        : "-"}
                    </div>
                  </div>
                </div>

                {meta.agenda ? (
                  <div className="mt-3 text-xs text-gray-600">
                    <span className="font-semibold text-gray-700">Agenda:</span>{" "}
                    {meta.agenda}
                  </div>
                ) : null}

                {meta.expected_output ? (
                  <div className="mt-1 text-xs text-gray-600">
                    <span className="font-semibold text-gray-700">Output:</span>{" "}
                    {meta.expected_output}
                  </div>
                ) : null}

                {meta.reference ? (
                  <div className="mt-1 text-xs text-gray-600">
                    <span className="font-semibold text-gray-700">Referensi:</span>{" "}
                    {meta.reference}
                  </div>
                ) : null}

                {row.description ? (
                  <div className="mt-3 whitespace-pre-wrap text-sm text-gray-700">
                    {row.description}
                  </div>
                ) : null}

                {latestResponse ? (
                  <div className="mt-3 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs text-emerald-700">
                    <span className="font-semibold">Respons terakhir:</span>{" "}
                    {latestResponse}
                  </div>
                ) : null}

                {canRespond ? (
                  <div className="mt-3 space-y-2">
                    <label className="block text-xs font-semibold text-gray-700">
                      « Tanggapan untuk perintah ini saja »
                    </label>
                    <p className="text-[11px] text-gray-500 -mt-1">
                      Dicatat pada tugas perintah ini (bukan pengajuan koordinasi baru).
                    </p>
                    <textarea
                      value={notes[row.id] || ""}
                      onChange={(e) =>
                        setNotes((prev) => ({
                          ...prev,
                          [row.id]: e.target.value,
                        }))
                      }
                      rows={3}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800"
                      placeholder="Jawaban atau ringkasan tindak lanjut khusus untuk perintah di kartu ini…"
                      aria-label="Tanggapan untuk perintah ini saja"
                    />
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      {allowClose ? (
                        <button
                          type="button"
                          onClick={() => handleClose(row.id)}
                          disabled={actingId === row.id}
                          className="px-3 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 text-sm font-semibold text-gray-700 disabled:opacity-60"
                        >
                          Tutup
                        </button>
                      ) : null}
                      <button
                        type="button"
                        onClick={() => handleRespond(row.id)}
                        disabled={actingId === row.id}
                        className="px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold disabled:opacity-60"
                      >
                        {actingId === row.id ? "Mengirim..." : "Kirim Tanggapan"}
                      </button>
                    </div>
                  </div>
                ) : allowClose && row.status !== "closed" ? (
                  <div className="mt-3 flex justify-end">
                    <button
                      type="button"
                      onClick={() => handleClose(row.id)}
                      disabled={actingId === row.id}
                      className="px-3 py-2 rounded-lg border border-gray-300 bg-white hover:bg-gray-50 text-sm font-semibold text-gray-700 disabled:opacity-60"
                    >
                      {actingId === row.id ? "Menutup..." : "Tutup"}
                    </button>
                  </div>
                ) : null}
              </div>
            );
          })}
        </div>
      )}
    </section>
  );
}
