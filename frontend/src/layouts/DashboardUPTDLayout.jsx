// frontend/src/layouts/DashboardUPTDLayout.jsx
        ...
        <>
          ...
          <div
            className={`
              ...
              ${
                isActive
                  ? "bg-blue-800/90 text-yellow-300"
                  : "bg-blue-700/85 text-blue-100 hover:bg-blue-800/85"
              }
...
export default function DashboardUPTDLayout({
  children,
  modules = uptdModules,
}) {
  ...
  const menuLabelMap = {
    DASHBOARD: "Dashboard UPTD",
    "UPT-TKN": "Layanan Teknis",
    "UPT-PGS": "Pengawasan & Sertifikasi",
    "UPT-LBR": "Laboratorium Uji",
    "UPT-SGD": "Sarana & Prasarana",
    "UPT-EVL": "Monitoring & Evaluasi",
    "UPT-LAP": "Laporan Kinerja",
    "UPT-KRD": "Koordinasi Lintas Sektor",
  };
  ...
  return (
    <div className="fixed inset-0 flex font-inter bg-gradient-to-br from-blue-900 via-blue-800 to-slate-800 text-slate-100 select-none">
      ...
      <aside
        className={`h-full bg-blue-950/95 z-30 flex flex-col items-center flex-shrink-0 transition-all duration-300 ${
          isMobile
      ...
      >
        ...
        <nav className="w-full flex flex-col gap-4 flex-1 justify-center px-3">
          {uptdMenu.length ? (
            ...
            <div className="px-2 py-4 text-center text-sm text-blue-100/70">
      ...
      <div className="flex-1 min-w-0 min-h-0 h-full flex flex-col bg-blue-950/80 backdrop-blur">
        <header className="flex-none w-full h-[75px] bg-blue-900/80 flex items-center px-4 md:px-12 shadow-sm z-10 sticky top-0">
          <button
            type="button"
            className="mr-3 inline-flex h-10 w-10 items-center justify-center rounded-lg border border-blue-700/70 bg-blue-950/60 text-blue-100 hover:bg-blue-800/70"
          ...
          <div className="text-lg md:text-2xl text-white font-bold tracking-wide flex-1">
            SIGAP <span className="font-light">·</span> UPTD Balai Pengawasan
          </div>
        ...
        <footer className="flex-none h-10 flex items-center text-xs justify-between px-10 w-full bg-gradient-to-r from-blue-900 to-blue-800/70 text-blue-100/80">
          ...
          <span>SIGAP Malut v1 | UPTD Balai Pengawasan</span>
        </footer>
      </div>
    </div>
  );
