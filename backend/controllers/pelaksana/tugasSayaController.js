  ...
  try {
    ...
    if (!task || task.status !== "assigned") {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Tugas tidak assigned atau sudah diterima",
        });
    }
...
export const mulaiTugas = async (req, res) => {
  ...
  try {
    ...
    if (!task || task.status !== "accepted") {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Tugas belum diterima" });
    }
...
export const submitTugas = async (req, res) => {
  ...
  try {
    ...
    if (!task || !["in_progress"].includes(task.status)) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Tugas belum siap disubmit" });
    }
...
export const revisiTugas = async (req, res) => {
  ...
  try {
    ...
    const task = await Task.findByPk(taskId, { transaction: t });
    if (!task || !["returned", "dikembalikan"].includes(task.status)) {
      await t.rollback();
      return res.status(400).json({
        success: false,
        message: "Tugas tidak dalam status dikembalikan",
      });
    }

    // Verifikasi tugas milik user ini
    const assignment = await TaskAssignment.findOne({
      where: { task_id: taskId, assignee_user_id: uid },
      transaction: t,
    });
    if (!assignment) {
      await t.rollback();
      return res.status(403).json({ success: false, message: "Tugas bukan milik Anda" });
    }
    ...
    if (atasanId) {
      await Notification.create({
        target_user_id: atasanId,
        task_id: taskId,
        message: `Tugas "${task.title}" sudah diperbaiki dan disubmit ulang oleh Pelaksana`,
      });
    }
...
export const createChecklistItem = async (req, res) => {
  try {
    ...
    const maxUrutan =
      (await SubChecklistTugas.max("urutan", {
...
export const toggleChecklistItem = async (req, res) => {
  try {
    ...
    if (!item || item.dibuat_oleh !== userId(req)) {
      return res
        .status(404)
        .json({ success: false, message: "Checklist tidak ditemukan" });
    }
