// =====================================================
// CONTROLLER: IndexController
// MODEL: Index
// Generated: 2026-03-19T23:39:27.523Z
// =====================================================

import Index from '../models/index.js';

// @desc    Get all Index records
// @route   GET /api/index
// @access  Private
export const getAllIndex = async (req, res) => {
  try {
    const { page = 1, limit = 10, search, ...filters } = req.query;
    
    const offset = (page - 1) * limit;
    
    const where = { ...filters };
    
    const { count, rows } = await Index.findAndCountAll({
      where,
      limit: parseInt(limit),
      offset: parseInt(offset),
      order: [['created_at', 'DESC']]
    });
    
    res.json({
      success: true,
      data: rows,
      pagination: {
        total: count,
        page: parseInt(page),
        limit: parseInt(limit),
        totalPages: Math.ceil(count / limit)
      }
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: 'Error fetching Index',
      error: error.message
    });
  }
};

// @desc    Get single Index by ID
// @route   GET /api/index/:id
// @access  Private
export const getIndexById = async (req, res) => {
  try {
    const record = await Index.findByPk(req.params.id);
    
    if (!record) {
      return res.status(404).json({
        success: false,
        message: 'Index not found'
      });
    }
    
    res.json({
      success: true,
      data: record
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: 'Error fetching Index',
      error: error.message
    });
  }
};

// @desc    Create new Index
// @route   POST /api/index
// @access  Private
export const createIndex = async (req, res) => {
  try {
    const record = await Index.create({
      ...req.body,
      created_by: req.user?.id
    });
    
    res.status(201).json({
      success: true,
      message: 'Index created successfully',
      data: record
    });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: 'Error creating Index',
      error: error.message
    });
  }
};

// @desc    Update Index
// @route   PUT /api/index/:id
// @access  Private
export const updateIndex = async (req, res) => {
  try {
    const record = await Index.findByPk(req.params.id);
    
    if (!record) {
      return res.status(404).json({
        success: false,
        message: 'Index not found'
      });
    }
    
    await record.update({
      ...req.body,
      updated_by: req.user?.id
    });
    
    res.json({
      success: true,
      message: 'Index updated successfully',
      data: record
    });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: 'Error updating Index',
      error: error.message
    });
  }
};

// @desc    Delete Index
// @route   DELETE /api/index/:id
// @access  Private
export const deleteIndex = async (req, res) => {
  try {
    const record = await Index.findByPk(req.params.id);
    
    if (!record) {
      return res.status(404).json({
        success: false,
        message: 'Index not found'
      });
    }
    
    await record.destroy();
    
    res.json({
      success: true,
      message: 'Index deleted successfully'
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: 'Error deleting Index',
      error: error.message
    });
  }
};
