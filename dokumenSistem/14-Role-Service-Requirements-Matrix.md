...
| 3 | Field mapping path mismatch (FIELDS_SEKRETARIAT/SEK-*.csv) | TINGGI | fieldMapping.js expect /FIELDS/FIELDS_*.csv | Fix path construction, scan FIELDS_SEKRETARIAT folder | Week 2 |
...
| 7 | Field mapping path convention (FIELDS_UPTD/ vs FIELDS/) | TINGGI | fieldMapping.js look for /FIELDS/FIELDS_*.csv, actual in FIELDS_UPTD/*.csv | Fix path construction or reorganize files | Week 2 |
...
Dokumen ini mendefinisikan standar akses, layanan, dan requirement untuk **8 role utama** dalam sistem SIGAP Malut. Setiap role memiliki:

- ✅ **Dashboard specification** (component, data freshness, export format)
- ✅ **Access matrix** (modul, Create/Read/Update/Delete/Approve/Finalize permission)
- ✅ **Security control checklist** (auth, authorization, audit, data protection)
- ✅ **Data/functional requirement** (kritical workflow, validation rule, integration)
- ✅ **Compliance issue list** (dengan severity, root cause, solution, timeline)

**Total 85 issue** teridentifikasi (18 KRITIS, 34 TINGGI, 33 SEDANG).

**Dengan sistematis remediation Phase 1-3** (16 minggu), sistem akan mencapai **78-85% compliance** terhadap dokumenSistem dan praktik terbaik pemerintah digital.

---

**Dokumen ini WAJIB direvisi setiap quarter untuk reflect perubahan requirement dan feedback dari real use.**
